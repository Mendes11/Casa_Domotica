...
class checkTomadas{
	...
	function __construct() {
		$this->columNames = array('iIDTomada','cNome','bStatus','iIDComodo');
    }
	...
	# Informa qual o conjunto de caracteres será usado.
		$this->sql = "Select iIDTomada,cNome,bStatus,iIDComodo from Tomadas";
		$response = array();
		...
		$myArray = array();
		$this->sql .= " where iIDComodo = ".$json_data[0]['iIDComodo'].";";
		...
		if(!empty($result)){
			if (mysqli_num_rows($result) > 0) {
				...
				if(sizeof($myArray) == sizeof($json_data)){
					$ret = self::checkArray($myArray,$json_data);
				}else{
				...
				if($response['hasDifference'] == true){
					...
					$response['errorMessage'] = "Dados referentes ao id ".$json_data['iIDTomada']." não foram encontrados...";
				}else{
					$response['hasDifference'] = false;
